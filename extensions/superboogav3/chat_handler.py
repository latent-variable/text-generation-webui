...
from extensions.superboogav3.utils import create_context_text, create_metadata_source
...
def custom_generate_chat_prompt_internal(user_input: str, state: dict, collector: ChromaCollector, **kwargs):
    if parameters.get_add_chat_to_data():
        ...
        # print('history_as_text:', history_as_text)
    ...
    if _should_query(user_input):
        ...
        results, meta = collector.get_sorted_by_dist(user_input, n_results=parameters.get_chunk_count(), max_token_count=int(parameters.get_max_token_count()), include_metadata=True )

        deduped_meta = []
        if meta:
            # Remove duplicates in the metadata
            
            duplicate_tracker = set()

            for dict in meta:
                if str(dict) not in duplicate_tracker:
                    duplicate_tracker.add(str(dict))
                    deduped_meta.append(dict)
            
            # save the metadata to a json file
            with open('metadata.json', 'w', encoding='utf-8') as f:
                json.dump(deduped_meta, f)
