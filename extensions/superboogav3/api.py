...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_get(self, search_strings: list[str], n_results: int, max_token_count: int, sort_param: str):
        ...
        if sort_param == parameters.SORT_DISTANCE:
            results, meta = self.collector.get_sorted_by_dist(search_string, n_results, max_token_count, True)
        elif sort_param == parameters.SORT_ID:
            results, meta = self.collector.get_sorted_by_id(search_string, n_results, max_token_count, True)
    ...
    def do_POST(self):
        try:
            ...
            if path in ['/api/v1/add', '/api/add']:
                ...
                # Check if the user wants to use the basic query
                basic_query = body.get('basic_query')
                if basic_query is not None:
                    print(f'serach_strings: {search_strings}, n_results: {n_results}')
                    results = self.collector.get_basic_by_dist(search_strings, n_results)
