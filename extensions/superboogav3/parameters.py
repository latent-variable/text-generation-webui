...
It expects a JSON file in `extensions/superboogav3/config.json`.
...
class Parameters:
    ...
    def __init__(self):
        if Parameters._instance is not None:
            ...
            self.hyperparameters = self._load_from_json(Path("extensions/superboogav3/config.json"))
...
def get_chunk_len() -> str:
    ...
    lens.append(Parameters.getInstance().hyperparameters['chunk_len1']['default'])
    lens.append(Parameters.getInstance().hyperparameters['chunk_len2']['default'])
    lens.append(Parameters.getInstance().hyperparameters['chunk_len3']['default'])
    lens.append(Parameters.getInstance().hyperparameters['chunk_len4']['default'])
...
def get_dist_threshold() -> float:
    return float(Parameters.getInstance().hyperparameters['dist_threshold']['default'])
