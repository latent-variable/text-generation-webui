...
os.environ['NLTK_DATA'] = str(Path("extensions/superboogav3/nltk_data").resolve())
...
def _begin_benchmark():
    score, max_score = benchmark(Path("extensions/superboogav3/benchmark_texts/questions.json"), collector)
...
def generate_html_for_json(data):
    if type(data) is list:
        html = '<ul>\n'
        for item in data:
            html += '<li>\n'
            html += generate_html_for_json(item)
            html += '</li>\n'
        html += '</ul>\n'
        ...
        for key, value in data.items():
            html += '<li>\n'
            if key == 'file_path':
                html += f'<strong>{key}:</strong> <a href="file:///{value}">{value}</a>'
            else:
                html += f'<strong>{key}:</strong> {generate_html_for_json(value)}'
            html += '</li>\n'
        html += '</ul>\n'
    else:
        html = str(data)
...
def load_metadata(file=r'./metadata.json'):
    try:
        ...
        html_tree = generate_html_for_json(metadata)
        ...
        logger.warn(f'Could not load metadata: {str(e)}')
    ...
    with gr.Blocks() as blocks:
        ...
        with gr.Row():
            with gr.Column(min_width=600):
                ...
                with gr.Tab("Source Data"):
                    with gr.Column():
                        # metadata = gr.Textbox(label='Source Data', placeholder='{"source": "my-source"}', lines=20, info='The metadata to be displayed here if in the generation', format='json')
                        metadata = gr.HTML(label="Source Data Tree Structure")

                        metadata_button = gr.Button('See Source Data')
        ...
        metadata_button.click(load_metadata, [], metadata)
...
if __name__ == '__main__':
    setup()
    blocks = ui()
    blocks.launch(server_port=1024, inbrowser=True, server_name= '0.0.0.0')